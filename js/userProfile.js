...
(async function userProfileFunc() {
  ...
  userCompany.textContent = `${data?.status} at ${data?.company}`;
  elUserLocation.textContent = data?.location;
  elUserBio.textContent = data?.bio;
  data.skills.forEach((one) => {
